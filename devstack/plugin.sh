...
probe python.function.entry = process("/usr/local/bin/python").library("libpython2.7.so.1.0").mark("function__entry")
{
...
EOF
    cat pyfunc_tmp.stp
    sudo LD_LIBRARY_PATH=/usr/local/lib stap -v pyfunc_tmp.stp -c "python -c 'pass'"
    bash $TOP_DIR/tools/install_pip.sh
    cd $old_dir
}
